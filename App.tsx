...
import {
    ...
    TutorialStep, TutorialState, DiplomacyState, Order, DiplomaticRelation
} from './types';
import { INITIAL_FACTIONS, INITIAL_PROVINCES, TECH_TREES, ROGUELIKE_TRAITS, BUILDINGS, DECREES, UNIT_TYPES, TUTORIAL_STEPS, REGIONS, createInitialDiplomacy } from './constants';
...
const TutorialOverlay = ({
    ...
    return (
        <div className="fixed inset-0 z-[100]">
            ...
            {/* Tutorial card */}
            <div className={`absolute ${positionClasses[step.position]} max-w-md w-full mx-4 pointer-events-auto animate-in fade-in slide-in-from-bottom-4 duration-500`}>
                <div className="bg-gradient-to-b from-amber-900/95 to-stone-900/95 border-2 border-amber-500/50 rounded-xl p-6 shadow-2xl shadow-amber-900/30">
                    {/* Header */}
                    <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">📜</span>
                        <div className="flex-1">
                            <h3 className="text-xl font-serif text-amber-200 font-bold leading-tight">{step.title}</h3>
                            <span className="text-amber-500/70 text-xs uppercase tracking-widest">Paso {stepNumber + 1} de {totalSteps}</span>
                        </div>
                    </div>

                    {/* Progress bar */}
                    <div className="h-1 bg-stone-800 rounded-full mb-4 overflow-hidden">
                        <div
                    ...
                    {/* Description */}
                    <p className="text-stone-300 leading-relaxed mb-6 text-sm">{step.description}</p>

                    {/* Action hint */}
                    {step.action && (
                        <div className="bg-amber-950/50 border border-amber-700/30 rounded-lg p-3 mb-4 flex items-center gap-2">
                            <span className="text-amber-400">👆</span>
                            <span className="text-amber-300 text-xs">Realiza la acción indicada para continuar</span>
                        </div>
                    )}
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        {!step.action && (
                            <button
                                onClick={onNext}
                                className="bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 px-6 py-2 rounded-lg font-bold text-white shadow-lg shadow-amber-900/30 transition-all hover:scale-105"
                            >
                                Siguiente →
                            </button>
                        )}
...
// Reinforcement Banner Component
const ReinforcementBanner = ({ count, onClose }: { count: number; onClose: () => void }) => {
    if (count <= 0) return null;

    return (
        <div className="fixed top-28 left-1/2 -translate-x-1/2 z-40 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="bg-gradient-to-r from-emerald-900/95 to-green-900/95 border-2 border-emerald-500/50 rounded-xl px-6 py-4 shadow-2xl shadow-emerald-900/30 flex items-center gap-4">
                <span className="text-4xl">🎲</span>
                <div>
                    <h4 className="text-emerald-200 font-serif font-bold text-lg">¡Refuerzos Disponibles!</h4>
                    <p className="text-emerald-400 text-sm">Tienes <span className="font-bold text-white">{count}</span> tropas para colocar. Haz clic en tus provincias.</p>
                </div>
                <button onClick={onClose} className="text-emerald-400 hover:text-white ml-4 text-xl">✕</button>
            </div>
        </div>
    );
};
...
function App() {
    const [gameState, setGameState] = useState<GameState>({
        ...
        diplomacy: {
            relations: createInitialDiplomacy(),
            treaties: []
        },
        orderQueue: [],
    ...
    const advanceTutorial = () => {
        setGameState(prev => {
            ...
            if (nextStep >= TUTORIAL_STEPS.length) {
                return {
                    ...prev,
                    tutorial: { ...prev.tutorial, isActive: false }
                };
            }
            return {
                ...prev,
                tutorial: {
                    ...
                    completedSteps: [...prev.tutorial.completedSteps, TUTORIAL_STEPS[prev.tutorial.currentStep].id]
                }
            };
        });
    };
    ...
    const skipTutorial = () => {
        ...
        addLog("Tutorial skipped. Access help from the pause menu.");
    };

    // Check if tutorial should advance based on player action
    const checkTutorialAction = (action: string) => {
        if (!gameState.tutorial.isActive) return;
        const currentStep = TUTORIAL_STEPS[gameState.tutorial.currentStep];
        if (currentStep?.action === action) {
            advanceTutorial();
        }
    };

    const startGame = (factionId: FactionId, withTutorial: boolean = false) => {
        ...
        // Calculate initial reinforcements (Risk-style)
        const ownedProvinces = INITIAL_PROVINCES.filter(p => p.ownerId === factionId);
        const initialReinforcements = calculateReinforcements(factionId, ownedProvinces);
        ...
        setGameState({
            ...
            logs: [`Welcome, leader of ${newFactions[factionId].name}.`, `You receive ${initialReinforcements} reinforcement troops!`],
            year: 270,
            turn: 1,
            isPaused: false,
            pendingReinforcements: initialReinforcements,
            tutorial: {
                isActive: withTutorial,
                currentStep: 0,
                completedSteps: []
            },
            diplomacy: {
    ...
    // Risk-style reinforcement calculation
    const calculateReinforcements = (factionId: FactionId, provinces: Province[]): number => {
        const ownedCount = provinces.filter(p => p.ownerId === factionId).length;

        // Base: 1 troop per 3 provinces (minimum 3)
        let reinforcements = Math.max(3, Math.floor(ownedCount / 3));

        // Bonus for complete regions
        REGIONS.forEach(region => {
            const regionProvinces = provinces.filter(p => region.provinceIds.includes(p.id));
            const allOwned = regionProvinces.every(p => p.ownerId === factionId);
            if (allOwned && regionProvinces.length === region.provinceIds.length) {
                reinforcements += region.bonusTroops;
            }
        });
    ...
    // Get controlled regions for display
    const getControlledRegions = (factionId: FactionId): string[] => {
        return REGIONS.filter(region => {
            const regionProvinces = gameState.provinces.filter(p => region.provinceIds.includes(p.id));
            return regionProvinces.length > 0 && regionProvinces.every(p => p.ownerId === factionId);
        }).map(r => r.name);
    };
    ...
    const selectProvince = (id: string | null) => {
        ...
        const targetProv = gameState.provinces.find(p => p.id === id);
        if (gameState.pendingReinforcements > 0 && targetProv && targetProv.ownerId === gameState.playerFactionId) {
            placeReinforcement(id);
        }

        setGameState(prev => ({ ...prev, selectedProvinceId: id }));

        // Check if selecting own province for tutorial
        if (targetProv && targetProv.ownerId === gameState.playerFactionId) {
            checkTutorialAction('SELECT_OWN_PROVINCE');
        }
    };
    ...
    const endTurn = () => {
        ...
        // Calculate reinforcements for player's next turn (Risk-style)
        if (gameState.playerFactionId) {
            const playerProvinces = updatedProvinces.filter(p => p.ownerId === gameState.playerFactionId);
            const reinforcements = calculateReinforcements(gameState.playerFactionId, updatedProvinces);
            const controlledRegions = getControlledRegions(gameState.playerFactionId);

            setTimeout(() => {
                setGameState(prev => ({
                    ...prev,
                    pendingReinforcements: reinforcements,
                    logs: controlledRegions.length > 0
                        ? [`🎲 Reinforcements: +${reinforcements} troops (Regions: ${controlledRegions.join(', ')})`, ...prev.logs]
                        : [`🎲 Reinforcements: +${reinforcements} troops`, ...prev.logs]
                }));
            }, 100);
        }

        checkTutorialAction('END_TURN');
    };

    // Place reinforcement troops in a province
    const placeReinforcement = (provinceId: string) => {
        if (gameState.pendingReinforcements <= 0) return;
        const prov = gameState.provinces.find(p => p.id === provinceId);
        if (!prov || prov.ownerId !== gameState.playerFactionId) return;
        ...
        const newUnit: Unit = {
            id: `reinforcement_${Date.now()}_${Math.random()}`,
            type: 'INFANTRY',
            name: 'Reinforcement',
            hp: 100,
            maxHp: 100,
            damage: 15,
            ownerId: gameState.playerFactionId!
        };

        setGameState(prev => {
            const newProvinces = prev.provinces.map(p =>
                p.id === provinceId ? { ...p, troops: [...p.troops, newUnit] } : p
            );
            return {
                ...prev,
                provinces: newProvinces,
    ...
    const recruitUnit = (type: UnitType) => {
        ...
        if (player.gold >= unitStats.cost && player.manpower >= unitStats.manpower) {
            ...
            checkTutorialAction('RECRUIT_UNIT');
        } else {
    ...
    const declareWarAttack = () => {
        ...
        checkTutorialAction('DECLARE_WAR');
                ...
                <div className="z-10 flex flex-col md:flex-row gap-8 w-full max-w-6xl h-[70vh] items-center">
                    ...
                    <div className="flex-1 h-[600px] flex flex-col pt-8">
                        ...
                        <div className="mt-auto flex flex-col gap-3">
                            <button onClick={() => startGame(previewFactionId!, true)} className="bg-gradient-to-b from-emerald-600 to-emerald-800 w-full max-w-md py-3 rounded-lg border-t border-emerald-400 shadow-[0_4px_20px_rgba(16,185,129,0.3)] text-white font-bold text-lg uppercase tracking-widest hover:scale-105 transition-transform flex items-center justify-center gap-3">
                                <span>📜</span> Iniciar con Tutorial
                            </button>
                            <button onClick={() => startGame(previewFactionId!, false)} className="bg-gradient-to-b from-[#f59e0b] to-[#ea580c] w-full max-w-md py-4 rounded-lg border-t border-[#fcd34d] shadow-[0_4px_20px_rgba(234,88,12,0.4)] text-white font-bold text-xl uppercase tracking-widest hover:scale-105 transition-transform flex items-center justify-center gap-4">
                                <span>⚔️</span> Comenzar Conquista
                            </button>
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="h-screen w-screen overflow-hidden bg-stone-950 font-sans text-stone-200 select-none">
                            ...
                            <div className="flex items-center gap-6 bg-[#1c1917]/80 backdrop-blur-md px-8 py-3 rounded-full border border-[#444] shadow-2xl">
                                ...
                                <div className="flex gap-6">
                                    <div className="flex flex-col items-center">
                                        <span className="text-orange-400 font-bold text-xl flex items-center gap-1"><span className="text-lg">☧</span> {gameState.factions[gameState.playerFactionId].gold}</span>
                                        <span className="text-[9px] text-stone-500 uppercase">Oro</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <span className="text-blue-400 font-bold text-xl flex items-center gap-1"><span className="text-lg">👥</span> {gameState.factions[gameState.playerFactionId].manpower}</span>
                                        <span className="text-[9px] text-stone-500 uppercase">Tropas</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <span className="text-purple-400 font-bold text-xl flex items-center gap-1"><span className="text-lg">🏛️</span> {gameState.provinces.filter(p => p.ownerId === gameState.playerFactionId).length}</span>
                                        <span className="text-[9px] text-stone-500 uppercase">Provincias</span>
                                    </div>
                                </div>
                            </div>
                            {/* Year/Turn indicator */}
                            <div className="bg-[#1c1917]/80 backdrop-blur-md px-6 py-3 rounded-full border border-[#444] shadow-2xl flex items-center gap-4">
                                <div className="flex flex-col items-center">
                                    <span className="text-amber-400 font-serif font-bold text-lg">{gameState.year} AC</span>
                                    <span className="text-[9px] text-stone-500 uppercase">Año</span>
                                </div>
                                <div className="w-px h-8 bg-[#444]"></div>
                                <div className="flex flex-col items-center">
                                    <span className="text-stone-300 font-bold text-lg">Turno {gameState.turn}</span>
                                    <span className="text-[9px] text-stone-500 uppercase">Ronda</span>
                                </div>
                            </div>
                        </div>

                        <div className="pointer-events-auto flex gap-4 mt-2">
                            {/* Pending reinforcements indicator */}
                            {gameState.pendingReinforcements > 0 && (
                                <div className="bg-emerald-900/80 backdrop-blur px-4 py-2 rounded-full border border-emerald-500 shadow-lg flex items-center gap-2 animate-pulse">
                                    <span className="text-lg">🎲</span>
                                    <span className="text-emerald-300 font-bold">+{gameState.pendingReinforcements}</span>
                                </div>
                            )}
                            ...
                            <button id="end-turn-btn" onClick={endTurn} className="w-14 h-14 rounded-full bg-orange-700 text-white flex items-center justify-center shadow-[0_0_20px_rgba(194,65,12,0.6)] hover:bg-orange-600 transition-transform hover:scale-110 border-2 border-orange-500 font-bold text-2xl" title="End Turn">➤</button>
                        </div>
                    </div>
                    {renderProvincePanel()}
                </>
            ...
            {gameState.pendingReinforcements > 0 && !gameState.tutorial.isActive && (
                <ReinforcementBanner
                    count={gameState.pendingReinforcements}
                    onClose={() => { }}
                />
            )}

            {/* Tutorial Overlay */}
            {gameState.tutorial.isActive && TUTORIAL_STEPS[gameState.tutorial.currentStep] && (
                <TutorialOverlay
                    step={TUTORIAL_STEPS[gameState.tutorial.currentStep]}
                    stepNumber={gameState.tutorial.currentStep}
